# CRNN_Tensorflow
This is a TensorFlow implementation of a Deep Neural Network for scene text recognition.  
It is  mainly based on the paper 
...
You can refer to the paper for architecture details.  
Thanks to the author [Baoguang Shi](https://github.com/bgshih).
  
The model consists of a **CNN stage** extracting features  
which are fed to an **RNN stage (Bi-LSTM)** and a **CTC loss**.

## Installation

This software has been developed on Ubuntu 16.04(x64) using python 3.5 and TensorFlow 1.12.  
Since it uses some recent features of TensorFlow it is incompatible with older versions.
...
- change code in requirements.txt  :>> tensorflow_gpu==**1.12.0**
- install cuda 8.0 https://developer.nvidia.com/cuda-80-ga2-download-archive

```
(crnntf)$ pip3 install -r requirements.txt
```

## Testing the pre-trained model

### Evaluate the model on the synth90k dataset
In this repo you will find a model pre-trained on the [Synth 90k](http://www.robots.ox.ac.uk/~vgg/data/text/)dataset.  

- Generate the tfrecords files (need the storage space - about 87 GB)
```
(crnntf)$ python ./tools/write_tfrecords.py -d /media/max/SSD/DATASET/CRNN/90kDICT32px/ -s /media/max/SSD/DATASET/CRNN/ -c ./data/char_dict_en.json -o ./data/char_dict/ord_map_en.json
```

When the tfrecords file of synth90k dataset has been successfully generated  
you may evaluated the model by the following script  
